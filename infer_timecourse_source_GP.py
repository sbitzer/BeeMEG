...
for sub in subjects:
    ...
    for hemi in ['L', 'R']:
        ...
        # fit the sparse GP
        gpm = fitgp(data.index.get_level_values('time').values, data.values, Z)
        
        # get the noiseless prediction
        gpmean, gpvar = gpm.predict_noiseless(xpred[:, None])
        gppred.loc[(sub, label, slice(None)), 'mean'] = gpmean[:, 0]
        gppred.loc[(sub, label, slice(None)), 'std'] = np.sqrt(gpvar[:, 0])
...
for ax, label in zip(axes, labels):
    labelpred = gppred.xs(label, level='label').unstack('subject')
